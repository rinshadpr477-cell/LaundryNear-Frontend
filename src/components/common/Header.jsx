  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-[#F8F7F4]/90 backdrop-blur-xl border-b border-black/5">
      ...
      <div className="h-20 px-5 md:px-12 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-8 text-xs font-medium uppercase text-[#111]">
          <Link to="/">Home</Link>
        ...
        <Link
          to="/"
          className="font-serif text-3xl md:text-4xl tracking-[-2px]"
        >
          LaundryNear
        </Link>
        ...
        <div className="hidden lg:flex relative">
          ...
          {/* <IoSearchOutline className="absolute left-4 top-3 text-[#6B4F3B]" /> */}
          ...
          {/* <input
            ...
            className="w-[320px] pl-10 pr-4 py-2 rounded-full border border-[#E5DED4] bg-white outline-none"
          /> */}

          {/* SUGGESTIONS DROPDOWN */}
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute top-12 w-full bg-white border rounded-xl shadow-lg overflow-hidden">

              {suggestions.map((shop) => (
                <div
                  key={shop.id}
                  onMouseDown={() => goToShop(shop.name)}
                  className="px-4 py-3 hover:bg-[#F5F1EB] cursor-pointer text-sm"
                >
              ...
              {profileOpen && (
                <div className="absolute right-0 top-12 w-52 bg-white border rounded-xl shadow-lg">
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-3 hover:bg-[#F5F1EB]"
                  >
                    Dashboard
                  </button>
