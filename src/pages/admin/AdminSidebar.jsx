...
function AdminSidebar() {
  ...
  const menuItems = [
    ...
    { name: "Shops", path: "/admin-shops", icon: <RiStore2Fill /> },
    ...
    <>
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 w-[260px] h-screen bg-[#101010] text-white px-5 py-6 flex-col border-r border-white/10 z-50">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.name} to={item.path} className={`group flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium duration-300 ${active ? "bg-white text-[#111111] shadow-sm" : "text-white/55 hover:bg-white/10 hover:text-white"}`}>
                <span className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg ${active ? "bg-[#F5F1EB] text-[#111111]" : "bg-white/5 text-white/55 group-hover:bg-white/10 group-hover:text-white"}`}>
                  {item.icon}
                </span>
        ...
        <div className="mt-auto bg-white/5 border border-white/10 rounded-2xl p-4">
          <p className="text-xs uppercase tracking-[3px] text-white/30 mb-2">
            System
          </p>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#101010] border-t border-white/10 z-50">
        <div className="grid grid-cols-6 py-3">
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.name} to={item.path} className={`flex flex-col items-center justify-center text-[10px] ${active ? "text-white" : "text-white/50"}`}>
