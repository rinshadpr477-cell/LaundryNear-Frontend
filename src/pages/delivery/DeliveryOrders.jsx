...

          {
            orders.length > 0 ? (
              ...
              orders.map((item) => (
                ...
                <div
                  key={item._id}
                  className="bg-white border border-[#E3D7C8] rounded-3xl p-6"
                >
                    ...
                    <div className="flex flex-col gap-3 lg:items-end">
...

                      {
                        item.status === "Ready for Delivery" && (
                          <button
                            onClick={() => handlePickupOrder(item._id)}
                            className="bg-[#1A1A1A] text-white px-5 py-3 rounded-2xl text-sm font-semibold hover:bg-[#3F2F24]"
                          >
                            Pick Up Order
                          </button>
                        )
